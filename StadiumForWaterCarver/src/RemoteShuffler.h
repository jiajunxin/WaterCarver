...
class RemoteShuffler {
  public:
	...
	RemoteShuffler(const vector<long>& config, vector<vector<Cipher_elg>* >* ciphers_in, ElGammal* reenc_key,
	        int m, int n, vector<vector<vector<long>* >* > * pi_in, vector<vector<ZZ>* > * R_in, bool owner = true);
	~RemoteShuffler();
