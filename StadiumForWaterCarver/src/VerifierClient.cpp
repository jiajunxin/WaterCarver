...
extern long mu;
extern long mu_h;
VerifierClient::VerifierClient(const vector<long> &config, int m, int n, vector<vector<Cipher_elg> *> *ciphers, vector<vector<Cipher_elg> *> *permuted_ciphers, ElGammal *elgammal, bool owner, bool do_process) : config_(config), c(ciphers), C(permuted_ciphers), time_v(0), owner_(owner), flow_flag_(false)
{
	m_r = config_[1] / mu;
	flow_flag_ = (m_r == 4);
...
string VerifierClient::round6red(const string &input_file, ZZ *challenge, ZZ *rand)
{
	...
	//cout << " before v round_6_red " << endl;
	//output_file = V->round_6_red(input_file, c, challenge, rand);
	//cout << " after v round_6_red " << endl;
	m_r = m_r / mu;
	auto tstop = high_resolution_clock::now();
	double ttime = duration<double>(tstop - tstart).count();
	//cout << "Time for round 6 " << ttime << " second(s)." << endl;
...
bool VerifierClient::round10red(const string &input_file)
{
	...
	auto tstop = high_resolution_clock::now();
	double ttime = duration<double>(tstop - tstart).count();
	//cout << "Time for round 10 red" << ttime << " second(s)." << endl;
	time_v += ttime;
