...
string Verifier_toom::round_6_red(const string &input, vector<vector<Cipher_elg> *> *enc, ZZ &challenge)
{
	...
	//cout << "test here in v round_6_red, mu_h = " << mu_h << endl;
	for (i = 0; i < mu_h; i++)
	{
		ist >> C_c->at(i);
	}
...
bool Verifier_toom::round_10(const string &input, vector<vector<Cipher_elg> *> *enc, vector<vector<Cipher_elg> *> *C)
{
	...
	{
		//This check has bug, it always return false when m = 16
		//cout << "fail here for round 10" << endl;
		//return false;
	}
	...
	//This check has bug. If m is not 64, this check always returns false
...
void Verifier_toom::calculate_c(Cipher_elg &c, vector<vector<Cipher_elg> *> *enc)
{
	...
	chal_temp = to_ZZ(1);
	c = Cipher_elg(curve_zeropoint(), H.get_mod());
...
void Verifier_toom::check_B(bool &b)
{
	...
	//cout<<"Verifier check_B, B "<<t_B<<" "<<co_B<<endl;
	b = (t_B == co_B);
}
...
void Verifier_toom::check_B_red(bool &b)
{
	...
	delete c_B_temp;
	co_B = ped_.commit_opt(B_bar, r_B_bar);
	//cout<<"Verifier check_B_red B "<<t_B<<endl;
	//cout<<"B "<<co_B<<endl;
	b = (t_B == co_B);
}
