...
extern string publicRandoms;

void Pedersen::set_public_vector(istringstream& f, long n, int o1, int o2, int o3) {
    if (publicRandoms.length() > 0) {
        f = istringstream(publicRandoms);
    }
    string line;
...
Pedersen::Pedersen(Mod_p* g, int n) {
	gen_prec = 0;
	if (NULL == g) {
		gen = nullptr;
	}
	gen = new vector<Mod_p>(n);
	int i = 0;
	for (i = 0; i < n ; i++) {
		if (G.is_generator(g[i])) {
			(*gen)[i] = g[i];
		} else {
			std::cout << "Error in Pedersen(Mod_p* g), invalid gen." << std::endl;
			gen = nullptr;
			return;
		}
	}
...
Mod_p Pedersen::commit(ZZ t,  ZZ ran){
	...
	ZZ mod = G.get_mod();
//	cout<< "Ped commit, mod = " << mod << "t = " <<t<<" ,ran = " <<ran<< endl;
//	cout<< "Ped commit, val 0  = " << gen->at(0).get_val() << endl;
//	cout<< "Ped commit, val 1  = " << gen->at(1).get_val() << endl;
