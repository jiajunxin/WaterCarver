...
// Retrieve the balance from the given address or 0 if object not found
func (self *StateDB) GetCTBalance(addr common.Address) crypto2.Key {
	stateObject := self.getStateObject(addr)
	if stateObject != nil {
		return stateObject.CTBalance()
	}
	return crypto2.Key{}
}
...
func (self *StateDB) SetCTBalance(addr common.Address, commitment crypto2.Key) {
	stateObject := self.GetOrNewStateObject(addr)
	if stateObject != nil {
		stateObject.SetCTBalance(commitment)
	}
}
